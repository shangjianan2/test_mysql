...
namespace NBIoT
{
    public partial class Form1 : Form
    {
        ...
        public void Init_DataBase_ListView(ListView listView_tt, ref ListViewItem[] lvi_tt)//MySqlHelper.GetDataSet是一个静态函数
        {
            ...
            {
                listView_tt.Columns.Add(temp_DataColumn[i].Caption);
            }

            //添加列
            lvi_tt = new ListViewItem[temp_DataRow.Count];
            for (int i = 0; i < temp_DataRow.Count; i++)
            {
                lvi_tt[i] = new ListViewItem();
                lvi_tt[i].ImageIndex = 0;
                lvi_tt[i].Text = temp_DataRow[i][0].ToString();
                for (int j = 1; j <= temp_DataColumn.Count - 1; j++)
                {
                    lvi_tt[i].SubItems.Add(temp_DataRow[i][j].ToString());
                }
            ...
            for (int i = 0; i < temp_DataRow.Count; i++)
            {
                listView_tt.Items.Add(lvi_tt[i]);
            }
